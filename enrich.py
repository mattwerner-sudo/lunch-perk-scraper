...
from config import OUTPUT_CSV, OUTPUT_ENRICHED_CSV, ENRICH_MIN_SCORE
...
_NON_US = re.compile(
    ...
    r"leuven|porto|portugal|lisbon|serbia|belgrade|bogot|medell|colombia|"
    r"bulgaria|sofia|stockholm|"
...
def export_dashboard_js(companies_df: pd.DataFrame, stats: dict, path: str = "dashboard_data.js"):
    ...
    # Drop heavy fields the dashboard doesn't use — office_cities can balloon
    # to ~100MB across all companies because location_lookup returns the full
    # office-city list for every domain ever billed.
    df = df.drop(columns=[c for c in ("office_cities", "location_detail") if c in df.columns])
    records = df.to_dict(orient="records")

    # Attach contacts (lazy import to avoid circular import at module load)
    import db as _db
    for r in records:
        name = r.get("company") or r.get("name") or ""
        r["contacts"] = _db.get_contacts(name) if name else []
    js = (
...
def run():
    ...
    # ── Contact enrichment (Sentrion-style decision-maker discovery) ─────────
